...
export default {
  async up(queryInterface, Sequelize) {
    ...
    if (!tableDescription.uid) {
      // Adicionar coluna uid permitindo nulos inicialmente
      ...
      // Criar índice para melhor performance nas consultas por UID
      await queryInterface.addIndex('tickets', ['uid'], {
        name: 'tickets_uid_index',
        unique: true
      });
    } else {
  ...
  async down(queryInterface, Sequelize) {
    ...
    if (tableDescription.uid) {
      ...
      try {
        await queryInterface.removeIndex('tickets', 'tickets_uid_index');
      } catch (error) {
        console.log('Índice tickets_uid_index não encontrado ou já removido');
      }
